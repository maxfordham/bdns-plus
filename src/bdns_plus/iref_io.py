from annotated_types import Ge, Annotated
import typing as ty
import logging
...
def serialize_iref(
    level_number: int, level_iref: Annotated[int, Ge(0)], level_min=-9, level_max=89
) -> Annotated[int, Ge(0)]:
...
def deserialize_iref(
    iref: Annotated[int, Ge(0)], level_min=-9, level_max=89
) -> tuple[ty.Union[int, None], ty.Union[Annotated[int, Ge(0)], None]]:
...
def get_next_iref(irefs: list[int], level_number: ty.Union[None, int] = None):
    if level_number is None:
        return max(irefs) + 1
    elif level_number is not None and len(irefs) == 0:
