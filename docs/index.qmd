...
| volume_level_instance | Instance      | +ve integer. a numerical indentifyer for a specific piece of equipment within a volume and level | #TODO: volume_level_instance
...
- **instance** properties (`volume`, `level` and `volume_level_instance`) in combination with `abbreviation`,
...
For legibility, the `unique_instance_reference` `MAY` be formatted with a delimiter (`instance_tag_delimiter`) between the `abbreviation`, `volume_reference`, `level_reference` and `volume_level_instance`.
...
- `abbreviation` = `LT`, `volume_id` = `1`, `level_id` = `0`, `volume_level_instance` = `1`, `instance_extra` = `E`, `instance_extra_delimiter` = `/`, `instance_tag_delimiter` = `_`
...
For a given `abbreviation`, the `volume_id`, `level_id` and `volume_level_instance` `MUST` be combined to create a unique identifier (`instance_id`) for each piece of equipment.
...
It `MUST` be possible to serialise and deserialise between [`volume_id`, `level_id` and `volume_level_instance`] and [`instance_id`] to create a unique identifier for each piece of equipment.

It `SHOULD` be simple to a user interogating the `instance_id` how to determine the `volume_id`, `level_id` and `volume_level_instance`.

- e.g. `instance_id` = `1001` would be `volume_id` = `1`, `level_id` = `0` and `volume_level_instance` = `1`.
