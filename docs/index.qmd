...
[BDNS (Building Device Naming Syntax)](https://theodi.github.io/BDNS/) ➕ **equipment tagging**.
`bdns-plus` extends BDNS to define a consistent and configurable approach to producing equipment references for Design Information (drawings, schedules, specifications).
...
- **WIP** `system_id` - the specific system that a piece of equipment belongs to. For electrical systems this parameter
...
| type              | Type          | refers to a specific equipment type used within a project                           |
| volume            | Instance      | indicates the volume within a project the equipment is situated                     |
| level             | Instance      | indicates the level within a project the equipment is situated                      |
| level_instance_id | Instance      | a numerical indentifyer for a specific piece of equipment within a volume and level |
| unique_instance_id| Instance      | calculated value from other instance parameters. used to create bdns tag            |


Parameters with property type, `Type` (`abbreviation` and `type`) would be the same for every identical item of equipment.
Parameters with property type, `Instance` (`volume`, `level` and `level_instance_id`), define a specific instance of
