---
title: Simple
format:
  html:
    code-fold: true
jupyter:
  jupytext:
    text_representation:
      extension: .qmd
      format_name: quarto
      format_version: '1.0'
      jupytext_version: 1.17.2
  kernelspec:
    display_name: Python 3 (ipykernel)
    language: python
    name: python3
---

In the examples below, we demonstrate how to generate and display asset tagging data for different types of equipment using the bdns-plus library.

## Configure Dummy Project

Below we generate some test data for a dummy project that has 1no volume and 5no levels. 

```{python}
#| echo: false
#| editable: true
%run _utils.py
```

::: {.callout-note collapse="true"}
## View shared code for generating docs examples. TODO - remove
```python
{{< include _utils.py >}}
```
:::


```{python}
#| editable: true
import pandas as pd
from bdns_plus.gen_idata import gen_config_iref

from bdns_plus.tag import bdns_tag, Tag
from bdns_plus.abbreviations import get_asset_abbreviations
from bdns_plus.gen_idata import batch_gen_idata, gen_config_iref
from bdns_plus.models import ConfigIref, GenDefinition, TTagData, ITagData, Config
from bdns_plus.docs import get_idata_tag_table, get_tags, get_light_fitting_types, get_electrical_accessory_types, get_electrical_distrubution_system, gen_project_equipment_data, display_tag_data

from IPython.display import Markdown
import random


LEVEL_MIN, LEVEL_MAX, NO_VOLUMES = -1, 3, 1
config_iref = gen_config_iref(LEVEL_MIN, LEVEL_MAX, NO_VOLUMES).model_dump()
df_levels = pd.DataFrame(config_iref["levels"])
df_volumes = pd.DataFrame(config_iref["volumes"])

display(Markdown(f"""
- level_min = {LEVEL_MIN}
- level_max = {LEVEL_MAX} 
- no_volumes = {LEVEL_MAX}"""))
display(Markdown("project config tables:"))
```

```{python}
LEVEL_MIN, LEVEL_MAX, NO_VOLUMES = -1, 3, 1

df = gen_project_equipment_data(level_min=LEVEL_MIN, level_max=LEVEL_MAX, no_volumes=NO_VOLUMES)
display_tag_data(df)
```

Version {{< var version >}} is a minor upgrade.

Please contact us at {{< var email.info >}}.

Quarto includes {{< var engine.jupyter >}} and 
{{< var engine.knitr >}} computation engines.

**Levels**

```{python}
#| echo: false
#| label: tbl-levels
#| editable: true
df_levels
```

**Volumes**

```{python}
#| echo: false
#| label: tbl-volumes
#| editable: true
df_volumes
```

## Electrical Distribution Equipment

For the main electrical distribution equipment a reference to every individual item is typically required,
and the default tagging procedure whereby the equipment is referenced by its level, volume and
level / volume instance is appropriate.


```{python}
#| editable: true
from great_tables import GT, html, style, loc
import polars as pl
import polars.selectors as cs


def get_electrical_distrubution_system(level_min=-1, level_max=3, no_volumes=1) -> list[ITagData]:
    config_iref = gen_config_iref(level_min, level_max, no_volumes)
    gen_def1 = GenDefinition(abbreviation=["PB"], no_items=1, on_levels=[0], on_volumes=None)  # 1 pb in GF
    gen_def2 = GenDefinition(abbreviation=["DB", "EM"], no_items=2, on_levels=None, on_volumes=None)  # 2 dbs / floor
    gen_def3 = GenDefinition(abbreviation=["DB", "EM"], no_items=2, on_levels=[0], on_volumes=None)  # 1 pb in GF
    gen_defs = [gen_def1, gen_def2, gen_def3]

    return batch_gen_idata(gen_defs, config_iref)

def get_electrical_accessory_types():
    """
    Generates a list of TTagData objects for predefined electrical accessories.

    Returns:
        list: A list of TTagData objects with abbreviations and type references.
    """
    abbreviations = ["DSSO", "DSO", "SSSO", "SSO", "FLRB", "INSO"]
    type_references = [4, 2, 3, 2, 3, 2]

    accessories = []
    for abbreviation, type_ref in zip(abbreviations, type_references):
        for n in range(1, type_ref):
            accessories.append(TTagData(abbreviation=abbreviation, type_reference=n))
    return accessories

def get_light_fitting_types():
    li = """LT
DL
EXIT
PL
SL""".split("\n")
    
    li_nos = [3, 3, 4, 4, 2] # [random.randint(2, 4) for n in range(0, len(li))]
    # ^ fixed random no's so git doesn't complain
    map_e = {0:"", 1: "E"}
    typs = []
    for abbreviation, type_ref in zip(li, li_nos):
        for n in range(1, type_ref):
            e = map_e[int(1/random.randint(1,5))]
            if abbreviation == "EXIT":
                e = "E"
            typs.append(TTagData(abbreviation=abbreviation, type_reference=n, type_extra=e))
    return typs

def get_tags(tag_data: TTagData | ITagData):
    
    tag = Tag(tag_data)
    if tag_data.__class__ == TTagData:
        return {
            "type_tag": tag.type,
        }
    else:
        return {
            "type_tag": tag.type,
            "instance_tag": tag.instance,
            "bdns_tag": tag.bdns,
        }

# Example usage
light_fittings = get_light_fitting_types()
import itertools
from IPython.display import display, Markdown, JSON
```


```{python}
from reactable import render

gt = display_tag_data(df)
# render(gt)
gt
```

import numpy as np
df["volume"].fillna(pd.NA).astype(int)

intcols

# df.to_numpy()

tag_data = di["electrical accessories"][0]
isinstance(d, TTagData)


tag = Tag(tag_data)
if tag.__class__ == TTagData:
    _ ={
        "type_tag": tag.type,
    }
else:
    _= {
        "type_tag": tag.type,
        "instance_tag": tag.instance,
        "bdns_tag": tag.bdns,
    }
_

tag.__class__ 

li[0]

d = di["electrical distribution"][0]
isinstance(d, TTagData)

idata

# TODO: delete
df = get_idata_tag_df(*get_idata_tag_table(idata))
display(df.style)

from bdns_plus.abbreviations import get_bdns_asset_abbreviations
bdns_abbr = list(get_asset_abbreviations().keys())

url_bdns = "https://github.com/theodi/BDNS/blob/master/BDNS_Abbreviations_Register.csv#L{index}"
df.columns = df.columns.droplevel()
df["type"] = ""
df["type_extra"] = ""
df["instance_extra"] = ""
df["url"] = [f"[{x}]({url_bdns.format(index=bdns_abbr.index(x)+2)})" for x in df.abbreviation.to_list()]
# cols = ['abbreviation', 'type', 'type_extra', 'level', 'level_iref', 'volume', 'asset_description',
#        'bdns_tag', 'type_tag', 'instance_tag', 
#        'instance_extra']
del df["asset_description"]
df["abbreviation"] = df["url"]
del df["url"]

# get_idata_tag_table(idata)
# df = pd.DataFrame()

display_tag_data(idata)

## Electrical Accessories

For electrical accessories such as sockets and switches, it is more common to simply indicate the type
and not bother referencing every individual instance.

electrical_accessories[0]

def get_electrical_accessory_types():
    """
    Generates a list of TTagData objects for predefined electrical accessories.

    Returns:
        list: A list of TTagData objects with abbreviations and type references.
    """
    abbreviations = ["DSSO", "DSO", "SSSO", "SSO", "FLRB", "INSO"]
    type_references = [4, 2, 3, 2, 3, 2]

    accessories = []
    for abbreviation, type_ref in zip(abbreviations, type_references):
        for n in range(1, type_ref):
            accessories.append(TTagData(abbreviation=abbreviation, type_reference=n))
    return accessories

# Example usage
electrical_accessories = get_electrical_accessory_types()

df_typs = pd.DataFrame([x.model_dump(mode="json") | {"asset_description": map_abbreviation_description[x.abbreviation.value]} | {"type_tag": Tag(x).type} for x in electrical_accessories])
df_typs = df_typs.fillna("")
df_typs.style

## Lighting

For light fixtures it is also typical to tag only by type, though it is common to also show additional info like if the fitting is emergency or not (see table below). 
At later stages, a light fitting may be uniquely identified to have specific commissioning values (e.g. a single spotlight on a control sytem, or a fitting that must be angled in a specific way), to achieve this just add the volume, level and level_iref parameters as shown in the []()value above.

df_typs = pd.DataFrame([x.model_dump(mode="json") | {"asset_description": map_abbreviation_description[x.abbreviation.value]} | {"type_tag": Tag(x).type} for x in light_fittings])
df_typs
