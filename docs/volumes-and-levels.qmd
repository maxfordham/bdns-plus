...
def mkgrid(df): return DataGrid(df.set_index("id"), column_widths={"name": 100}) #, layout={"width":"40%"}
...
w.HBox([
  w.VBox([w.HTML("<b>Volumes</b>"), gr_volumes], layout={"width": "50%"}), 
  w.VBox([w.HTML("<b>Levels</b>"), gr_levels], layout={"width": "50%"}),
])
``` -->
...
BDNS compliant unique_instance_id.
...
or for a more complex example, requiring mapping to "id" to create a compliant BDNS tag:
...
- add encodings for things like mezzanine levels
...
volumes = to_records(list(csv.reader(_volumes.split("\n"))))
levels = to_records(list(csv.reader(_levels.split("\n"))))

config = Config(volumes=volumes, levels=levels)
